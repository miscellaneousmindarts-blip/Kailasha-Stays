-- =============================================================================
-- Room service menu (link and/or PDF) per property
--
-- Shown on the public listing page and on the guest portal, right after
-- payment details and the ID upload section — not buried at the bottom,
-- since it's something a checked-in guest actually needs in the moment.
--
-- Link and PDF are independent: a property can have either, both, or
-- neither. No CHECK requiring one or the other — an empty state is a normal,
-- valid state for a new listing.
--
-- Safe to re-run.
-- =============================================================================

alter table public.properties
  add column if not exists room_service_link text,
  add column if not exists room_service_pdf_path text;

comment on column public.properties.room_service_link is
  'External URL to a room-service menu (e.g. a hosted doc or ordering site). Independent of room_service_pdf_path.';
comment on column public.properties.room_service_pdf_path is
  'property-documents bucket path to an uploaded PDF menu. Independent of room_service_link.';

-- -----------------------------------------------------------------------------
-- property-documents: a bucket of its own rather than a folder inside
-- property-images, mirroring why homepage-media got its own bucket in 0008 —
-- this holds PDFs, not photos/videos, so the allowed_mime_types list stays a
-- meaningful gate rather than a lie.
--
-- Public, like property-images: the menu must be visible to a site visitor
-- who hasn't booked yet, not just a confirmed guest.
--
-- Write policies are NOT yet tenant-scoped, matching property-images and
-- homepage-media today — that gap is tracked platform-wide for phase B8,
-- which re-scopes all three buckets by tenant-prefixed path in one pass
-- rather than fixing them one at a time.
-- -----------------------------------------------------------------------------

insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values ('property-documents', 'property-documents', true, 10485760, array['application/pdf'])
on conflict (id) do update
  set public = excluded.public,
      file_size_limit = excluded.file_size_limit,
      allowed_mime_types = excluded.allowed_mime_types;

drop policy if exists property_documents_public_read on storage.objects;
create policy property_documents_public_read on storage.objects
...
create policy property_documents_admin_write on storage.objects
  for insert to authenticated
  with check (bucket_id = 'property-documents' and public.is_admin());

drop policy if exists property_documents_admin_update on storage.objects;
create policy property_documents_admin_update on storage.objects
  for update to authenticated
  using (bucket_id = 'property-documents' and public.is_admin());

drop policy if exists property_documents_admin_delete on storage.objects;
create policy property_documents_admin_delete on storage.objects
  for delete to authenticated
  using (bucket_id = 'property-documents' and public.is_admin());

-- -----------------------------------------------------------------------------
-- get_booking_by_token: 'documents' entries gain 'id' (needed so the guest
-- can view/edit/remove a specific upload within the 30-minute window — the
-- RPC previously returned nothing that identified *which* row a document
-- was), and 'property' gains the two room-service columns above. Every other
-- line is reproduced verbatim from 0016_guest_portal_branding.sql.
-- -----------------------------------------------------------------------------
