 ...
 *
 * The one deliberate divergence from lib/homepage.ts, per
 * docs/apex-homepage-editor-plan.md §3: every schema below `.default()`s to
 * the EXACT copy already hardcoded in components/platform/*.tsx and
 * lib/platform-content.ts, not to an empty string. There is one apex, and
 * that copy — with the provenance comments explaining why the reviews name
 * no source platform, why the comparison table's "Cost: Mid" row must stay
 * a concession rather than a claim — is worth keeping as the code default
 * rather than discarding into a seed. A row's content is `{}` until edited;
 * applying 0029 and reading through this file changes nothing visible.
 */
...
/* Section schemas                                                    */
/*                                                                     */
/* Order and keys match the seed in 0029 and app/(platform)/page.tsx's */
...
export const platformSectionSchemas = {
  hero: z.object({
    ...
    // — see resolveHero() below. Uploading one here takes priority.
    imageId,
    trustItems: z
...
/* ------------------------------------------------------------------ */
/* Icon registries — shared between resolvers (none needed above) and */
/* the editors (Task #14), so the enum values and their glyphs stay   */
